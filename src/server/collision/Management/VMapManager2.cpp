	...
	*/
	...
	WorldModel* VMapManager2::acquireModelInstance(const std::string& basepath, const std::string& filename)
	{
		...
		if (model == iLoadedModelFiles.end())
		{
			...
			{
				sLog->outError(LOG_FILTER_MAPS, "VMapManager2: could not load '%s%s.vmo'", basepath.c_str(), filename.c_str());
				delete worldmodel;
				return NULL;
			}
			sLog->outError(LOG_FILTER_MAPS, "VMapManager2: loading file '%s%s'", basepath.c_str(), filename.c_str());
	...
	void VMapManager2::releaseModelInstance(const std::string &filename)
	{
		...
		if (model == iLoadedModelFiles.end())
		{
			sLog->outError(LOG_FILTER_MAPS, "VMapManager2: trying to unload non-loaded file '%s'", filename.c_str());
			return;
		}
		if (model->second.decRefCount() == 0)
		{
			sLog->outError(LOG_FILTER_MAPS, "VMapManager2: unloading file '%s'", filename.c_str());
