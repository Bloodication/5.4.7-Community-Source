...
PathGenerator::PathGenerator(const Unit* owner) :
    ...
    _navMeshQuery(NULL)
{
    //sLog->oUTmA("maps", "++ PathGenerator::PathGenerator for %u \n", _sourceUnit->GetGUIDLow());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
   ...
   // TC_LOG_DEBUG("maps", "++ PathGenerator::CalculatePath() for %u \n", _sourceUnit->GetGUIDLow());
    ...
    if (!_navMesh || !_navMeshQuery || _sourceUnit->HasUnitState(UNIT_STATE_IGNORE_PATHFINDING) ||
        !HaveTile(start) || !HaveTile(dest)) /*|| _sourceUnit->GetTransport()*/
    {
        BuildShortcut();
...
void PathGenerator::BuildPolyPath(Vector3 const& startPos, Vector3 const& endPos)
{
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            //C_LOG_ERROR("maps", "%u's Path Build failed: 0 length path", _sourceUnit->GetGUIDLow());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
