...
bool PathGenerator::InValidPathZ(Unit* target)
{
    ...
    return (std::fabs(target->GetPositionZ() - endPos.z) > 5.f);
}

void PathGenerator::SetPositionZ(float z)
{
    float new_z = _pathPoints[_pathPoints.size()-1].z;
    float x = _pathPoints[_pathPoints.size()-1].x;
    float y = _pathPoints[_pathPoints.size()-1].y;
    
    if (z - new_z > 1.f)
        new_z = z;
      
   _pathPoints[_pathPoints.size()-1] = G3D::Vector3(x, y, new_z);
}
