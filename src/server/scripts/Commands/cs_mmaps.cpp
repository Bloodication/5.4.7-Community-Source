...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "loadedtiles", SEC_PLAYER, false, &HandleMmapLoadedTilesCommand, "", NULL },
			{ "loc", SEC_PLAYER, false, &HandleMmapLocCommand, "", NULL },
			{ "path", SEC_PLAYER, false, &HandleMmapPathCommand, "", NULL },
			{ "stats", SEC_PLAYER, false, &HandleMmapStatsCommand, "", NULL },
			{ "testarea", SEC_PLAYER, false, &HandleMmapTestArea, "", NULL },
            { NULL,          0,                                  false, NULL,                          "", NULL }
        };

        static ChatCommand commandTable[] =
        {
			{ "mmap", SEC_PLAYER, true, NULL, "", mmapCommandTable },
            { NULL,   0,                     false, NULL, "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleMmapPathCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 i = 0; i < pointPath.size(); ++i)
            player->SummonCreature(3123, pointPath[i].x, pointPath[i].y, pointPath[i].z, 0, TEMPSUMMON_TIMED_DESPAWN, 9000);
